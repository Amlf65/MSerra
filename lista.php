        ...
        <div class="row mb-5">
          ...
          if (isset($_GET['orden'])) {
            if (isset($_GET['limite'])) {
              $resultado = $conexion->query("select * from productos where inventario>0 order by " .$_GET['orden']." ,id limit " . $_GET['limite'] . "," . $limite) or die($conexion->error);
            } else {
              $resultado = $conexion->query("select * from productos where inventario>0 order by " .$_GET['orden']. ",id limit " . $limite) or die($conexion->error);
            }
          } else {
            if (isset($_GET['limite'])) {
              $resultado = $conexion->query("select * from productos where inventario>0 order by tipo,id limit " . $_GET['limite'] . "," . $limite) or die($conexion->error);
            } else {
              $resultado = $conexion->query("select * from productos where inventario>0 order by tipo,id limit " . $limite) or die($conexion->error);
            }
          }
